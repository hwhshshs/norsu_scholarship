...
## Legacy Scholarship Integration
The legacy scholarship management system has been integrated under `public/legacy`.

1. Import the legacy schema and seed data:

    ```bash
    mysql -u root -p schorlarship_main < "public/legacy/DATABASE FILE/schoolfeesys.patched.sql"
    ```

2. Start Laravel and sign in:

    ```bash
    php artisan serve
    ```

3. Open the integrated launcher:
    - `/scholarship-system` (via Laravel route)
    - `/legacy/login.php` (direct legacy login)

By default, legacy pages use:
- `DB_HOST=localhost`
- `DB_USER=root`
- `DB_PSWD=` (empty)
- `DB_NAME=schorlarship_main`
