  ...
  <div class="col-12">
    <div class="card mb-4">
      <div class="card-body d-md-flex align-items-center justify-content-between">
        <div>
          <h5 class="mb-1">Semester Management</h5>
          <p class="text-sm mb-0">Create, update, activate, deactivate, and safely remove semester records.</p>
        </div>
        <div class="mt-3 mt-md-0 d-flex flex-wrap">
          <a href="{{ route('scholarship-academic.semesters.create') }}" class="btn bg-gradient-primary mb-0 me-2">Add Semester</a>
  ...
  <div class="col-12">
    <div class="card mb-4">
      ...
      <div class="card-body pt-3">
        <form method="GET" action="{{ route('scholarship-academic.semesters.index') }}" class="row g-3 align-items-end">
          <div class="col-md-6">
            <label class="form-label">Search</label>
            <input type="text" name="q" class="form-control" value="{{ $search }}" placeholder="Semester label" />
          </div>
          <div class="col-md-3">
          ...
          <div class="col-md-3 d-flex">
            ...
            <a href="{{ route('scholarship-academic.semesters.index') }}" class="btn btn-outline-secondary mb-0">Reset</a>
          </div>
        </form>
      </div>
    </div>
...
<div class="row">
  <div class="col-12">
    <div class="card">
      <div class="card-header pb-0">
        <h6 class="mb-0">Semester Records</h6>
      </div>
              ...
              @forelse ($rows as $row)
                ...
                <tr>
                  ...
                  <td class="text-end pe-3">
                    <a href="{{ route('scholarship-academic.semesters.edit', $row->id) }}" class="btn btn-link text-dark px-2 mb-0">Edit</a>

                    <form method="POST" action="{{ route('scholarship-academic.semesters.toggle-status', $row->id) }}" class="d-inline">
                      @csrf
                    ...
                    <form method="POST" action="{{ route('scholarship-academic.semesters.remove', $row->id) }}" class="d-inline">
                      @csrf
                      <button type="submit" class="btn btn-link text-danger px-2 mb-0" onclick="confirmAction(event, 'Remove Semester?', 'Are you sure you want to proceed with this removal?');">{{ $isInactive ? 'Delete' : 'Remove' }}</button>
                    </form>
                  </td>
                </tr>
              @empty
                <tr>
                  <td colspan="4" class="text-center text-sm text-secondary py-4">No semesters found for the selected filters.</td>
                </tr>
              @endforelse
            </tbody>
          </table>
